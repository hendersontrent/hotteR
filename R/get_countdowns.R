...
#' and return a dataframe of cleaned results in an analysis-ready format
#'
#' @import dplyr
#' @importFrom magrittr %>%
#' @import glue
...
get_countdowns <- function(){
  ...
  # Fire up Selenium ----
  rD <<- rsDriver(browser = "chrome", chromever = "87.0.4280.88", verbose = FALSE, port = 4444L)
  remDr <<- rD[['client']]
  ...
  country_list <- extract_dropdown_options('//*[@id="main"]/div[3]/div/div/div[5]/label/select')
  year_list <- extract_dropdown_options('//*[@id="main"]/div[3]/div/div/div[1]/label/select')
  ...
  for(yr in seq(2, 36)){
    ...
    tibble_triple_j <- tibble()

    click_element(glue('//*[@id="main"]/div[3]/div/div/div[1]/label/select/option[{yr}]'))
    this_year <- year_list[yr-1]
    print(glue('====================== {this_year} ======================'))

    # Loop through the 20 countries to find songs for each year ----
    for(c in seq(2, 20)){
      click_element(glue('//*[@id="main"]/div[3]/div/div/div[5]/label/select/option[{c}]'))

      this_country <- country_list[c-1]
      print(glue('Now on {this_country} ...'))
      ...
      for(i in seq(1:100)){ # each country can at most have 100 songs
        ...
        suppressMessages({
          tryCatch({
            rank <- extract_element_text(glue('//*[@id="main"]/div[4]/div/table/tbody/tr[{i}]/td[2]'))
            artist <- extract_element_text(glue('//*[@id="main"]/div[4]/div/table/tbody/tr[{i}]/td[3]'))
            song <- extract_element_text(glue('//*[@id="main"]/div[4]/div/table/tbody/tr[{i}]/td[4]'))
            if(rank == ""){
              print(glue("No songs for {this_country}. Move on."))
              break() # if no song, move on to next country
            }
            tibble_triple_j <- bind_rows(tibble_triple_j, tibble(rank = rank %>% as.numeric(),
                                                                 artist = artist,
                                                                 song = song,
                                                                 year = this_year,
    ...
    if(nrow(filter(tibble_triple_j, year == this_year)) != 100){
      stop("Didn't successfully scrape 100 songs for this year. Something must have gone wrong.")
    } else{
      print(glue("===================================================="))
    }
    store_here[[yr-1]] <- tibble_triple_j %>% arrange(rank)
   }
   all_tibble_triple_j <- data.table::rbindlist(store_here, use.names = TRUE)

   fin_time <<- Sys.time()
   message(glue("Scraping completed! Time elapsed: {round(as.numeric(fin_time - start_time) * 60)} minutes."))
